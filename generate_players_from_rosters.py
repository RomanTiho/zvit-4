...
for team in teams:
    ...
    for i, name in enumerate(team.player_roster):
        ...
        player = Player.objects.create(user=user, position=position, jersey_number=random.randint(1, 99))
        created_count += 1
        ...
        for m in range(num_matches):
            PlayerStats.objects.create(
                ...
                shots_on_target=random.randint(0, 3) if position in ['FWD', 'MID'] else 0,
