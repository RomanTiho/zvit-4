...
class UserProfile(models.Model):
    """Профіль користувача з роллю та аватаром"""
    ...
    def __str__(self):
        role = "Тренер" if self.is_coach else "Користувач"
        return f"{self.user.username} ({role})"


@receiver(post_save, sender=User)
def create_user_profile(sender, instance, created, **kwargs):
    """Автоматично створити UserProfile при реєстрації"""
    if created:
...
class Player(models.Model):
    ...
    jersey_number = models.IntegerField(
        null=True, blank=True, help_text="Ігровий номер (1-99)"
    )
...
class PlayerStats(models.Model):
    ...
    shots_on_target = models.IntegerField(
        default=0, help_text="Удари у ціль (xG proxy)"
    )
    key_passes = models.IntegerField(
        default=0, help_text="Ключові передачі (передача під удар)"
    )
    ...
    def _calculate_match_rating(self):
        ...
        sot_bonus = min(
            Decimal(str(self.shots_on_target)) * Decimal("0.2"), Decimal("1.5")
        )
        score += sot_bonus
        ...
        if position in ("GK", "DEF", "MID"):
            def_bonus = min(
                (Decimal(str(self.tackles)) + Decimal(str(self.interceptions)))
                * Decimal("0.1"),
                Decimal("1.0"),
            )
            score += def_bonus
...
class UPLSquadCache(models.Model):
    ...
    team_name = models.CharField(
        max_length=100, unique=True, verbose_name="Назва команди"
    )
...
class Tournament(models.Model):
    ...
    image = models.ImageField(
        upload_to="tournaments/", null=True, blank=True, verbose_name="Фото турніру"
    )
