...
class IsCoach(BasePermission):
    ...
    def has_permission(self, request, view):
        return (
            ...
            and request.user.groups.filter(name="Coach").exists()
        )
...
class AuthViewSet(viewsets.ViewSet):
    ...
    def request_password_reset(self, request):
        ...
        if not email:
            return Response(
                {"error": "Вкажіть email"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            return Response(
                {
                    "error": "Не вдалося надіслати лист. Перевірте налаштування поштового сервера або спробуйте пізніше."
                },
    ...
    def confirm_password_reset(self, request):
        ...
        if not uidb64 or not token:
            return Response(
                {"error": "Невірне посилання для скидання пароля"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        if not new_password:
            return Response(
                {"error": "Вкажіть новий пароль"}, status=status.HTTP_400_BAD_REQUEST
            )

        if new_password != confirm_password:
            return Response(
                {"error": "Паролі не співпадають"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if not token_generator.check_token(user, token):
            return Response(
                {"error": "Посилання для скидання пароля недійсне або прострочене"},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def upload_avatar(self, request):
        ...
        if "avatar" not in request.FILES:
            return Response(
                {"error": "Файл не знайдено. Передайте поле avatar."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if file.size > 5 * 1024 * 1024:
            return Response(
                {"error": "Розмір файлу не може перевищувати 5 МБ."},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def coach_stats(self, request):
        ...
        recent_tournaments = Tournament.objects.filter(id__in=tournament_ids).order_by(
            "-start_date"
        )[:5]
        recent_data = [
            {
...
def upl_sync_view(request):
    ...
    if not request.user.is_staff:
        return Response(
            {"detail": "Тільки адміністратори."}, status=status.HTTP_403_FORBIDDEN
        )
