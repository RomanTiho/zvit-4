...
SECRET_KEY = os.environ.get("SECRET_KEY", "django-insecure-k2_vpxt!1j3*_oqqxm0dng-*yn8y4ktgazs$i(^3nm#kqhx@+^")
...
# DATABASES is defined in each environment-specific settings file:
#   local.py  → MySQL (existing local setup)
#   prod.py   → PostgreSQL via DATABASE_URL (Render)
...
if _email_backend_mode == "console":
    ...
    EMAIL_USE_TLS = _os.environ.get("EMAIL_USE_TLS", "true").strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
    ...
    DEFAULT_FROM_EMAIL = _os.environ.get(
        "DEFAULT_FROM_EMAIL", EMAIL_HOST_USER or "no-reply@footballhub.local"
    )
